...
namespace Paymentwall
{
	public class PWBrickForm : MonoBehaviour {
		...
		public void UpdateFormInformation (string merchant, string product, double amount, string currency) {
			...
			transform.FindChild ("Container/Panel/Brick Merchant Name").GetComponent<Text> ().text = merchant;
			transform.FindChild ("Container/Panel/Product Name").GetComponent<Text> ().text = product;
			transform.FindChild ("Container/Panel/Button Pay/Text").GetComponent<Text> ().text = "Pay " + amount.ToString ("C2") + " " + currency;
		}
			...
			if (www.error == null) {
				...
				if (response != null) {
					if (response ["type"] != "Error") {
						...
						buttonPay.transform.FindChild ("Text").GetComponent<Text> ().text = ".....";
			...
			if (www.error == null) {
				...
				if (response != null) {
					...
					if (objMessage.Equals ("charge")) {
						...
						buttonPay.transform.FindChild ("Text").gameObject.SetActive (false);
						...
						buttonPay.transform.FindChild ("Text").GetComponent<Text> ().text = diagMess;
					}
				} else {
					txtErrorEmail.SetActive(true);
					txtErrorEmail.GetComponent<Text>().text = www.error;
